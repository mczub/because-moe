...
$(document).ready(function(){
	...
	function sortshowlist(){
		...
		for (i=0; i < length; i++) {
	         ...
	         */
	        for (j=i-1; j > -1 && shows[j].ratings < value.ratings; j--) {
	            shows[j+1] = shows[j];
	        }
	...
	function updateShowList(showList, message){
		...
		if (showList.length == 0){
			...
			showList.forEach(function(show){
				var resultHtml;
				if (show.sites.MyAnimeList) {
					resultHtml = "<div class='result'><div class='result-name'><a href=\"" + show.sites.MyAnimeList + "\" target=\"_blank\">" + show.name + "</a>";
				}
				else {
					resultHtml = "<div class='result'><div class='result-name'>" + show.name;
				}
				if (show.alt){
